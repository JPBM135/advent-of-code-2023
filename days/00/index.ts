...
function getFirstAndLastDigit(line: string) {
  const numbersFromLine = line.matchAll(/\d/g);

  const numberArray = Array.from(numbersFromLine).map((match) => match[0]);

  const firstDigit = numberArray.at(0);
  const lastDigit = numberArray.at(-1);

  return { last: lastDigit, first: firstDigit };
}

let sumPartOne = 0;

for (const line of lines) {
  const { last, first } = getFirstAndLastDigit(line);

  const finalNumber = Number(String(first) + String(last));

  sumPartOne += finalNumber;
}

Logger.success(0, 1, `Sum of all of the calibration values: ${sumPartOne}`);

function getFirstAndLastDigitConsideringWritten(line: string) {
  const writtenNumbers = ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine'];

  const matchRegex = new RegExp(`\\d|(${writtenNumbers.slice(1).join(')|(')})`, 'g');

  const numbersFromLine = line.matchAll(matchRegex);

  const numberArray = Array.from(numbersFromLine).map(([match]) => {
    const isNumber = Number.isInteger(Number(match));
    return isNumber ? Number(match) : writtenNumbers.indexOf(match);
  });
...
for (const line of lines) {
  const { last, first } = getFirstAndLastDigitConsideringWritten(line);

  Logger.debug(`${line}`, { first, last });

  const finalNumber = Number(String(first) + String(last));

  sumPartTwo += finalNumber;
}

Logger.success(0, 2, `Sum of all of the calibration values considering written numbers: ${sumPartTwo}`);
